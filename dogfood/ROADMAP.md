...
- **#4 (open, Wave 3):** `da index query --env preview` silently queries **live**. `--env` is a deprecated alias shadowed by the global `--env dev|stage|prod`, so the value never reaches the command and it defaults to live. `--target preview` works. Fix: remove/error the un-satisfiable `--env` alias on index commands.
- **#5 (open, Wave 3):** `da route classify` labels live code assets (e.g. `/scripts/aem.js`, `code.status: 200`) as **orphan** — it infers ownership only from a content route's `sourceLocation` and never reads the admin `code` status. Fix: fold `code.status` into the classifier so `codebus`/`hybrid` reflect real code-bus presence.
- **#6 (open, Wave 3):** after a source delete, `da route classify` still returns **contentbus** for the orphaned live artifact (trusts stale `sourceLocation` over its own `daSource: false`). Fix: `daSource:false` + live artifact ⇒ orphan.
