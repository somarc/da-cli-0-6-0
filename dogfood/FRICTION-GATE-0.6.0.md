...
**Status:** **friction column clear** (f015 Critical–f033 fixed/recovered + locked + pushed 2026-07-17; f013 upgraded + fixed; f017 partial with seams landed and migration tracked; f022–f026 are the Wave 4 completion haul; **f027–f032 are Wave 5's haul** — six findings from five drills, every drill surfaced something real; **f033** is the ops-turn catch — a stale PATH install reproduced already-fixed bugs under the same 0.5.1 version string, `da status` now reports binary provenance, and the f021-residue `publish pages` variadic asymmetry closed in the same pass) — wave column: **Wave 4 cut 2026-07-16**, **Wave 5 in progress** (A1, A2, A3, C1, C3 closed; C2 recognition+containment done, unpublish leg pending permission), 6 open  
...
- f017 remainder: migrate config/auth/site create and the rest of the command surface onto the envelope seams (deploy pages closed by f026; content read-listing closed by f027; publish pages dry-run closed by f033's pass — committed preview/publish pages success arrays still on the march).
